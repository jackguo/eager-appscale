...
API specification and dependency metadata (c.f. Section~\ref{sec:adc}),
provides support for the Java language.  Developers document APIs and their
dependences manually for web services written in other languages.

Like most PaaS technologies, AppScale includes an application deployment
service that distributes, launches, exports an application
as a web-accessible service.  EAGER controls this deployment
process according to the policies that the platform adminstrator specifies.

\subsection{Developer Tools for EAGER}
EAGER provide two developer tools for implementing applications and APIs for an
